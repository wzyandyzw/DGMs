...
- For the optimal discriminator \( D^*_G(\cdot) \), we have
  $$
  \begin{aligned}
  V(G, D^*_G) &= \mathbb{E}_{x \sim p_{\text{data}}} \left[ \log \left( \frac{p_{\text{data}}(x)}{p_{\text{data}}(x) + p_G(x)} \right) \right] + \mathbb{E}_{x \sim p_G} \left[ \log \left( \frac{p_G(x)}{p_{\text{data}}(x) + p_G(x)} \right) \right] \\  &= \mathbb{E}_{x \sim p_{\text{data}}} \left[ \log \left( \frac{p_{\text{data}}(x)}{(p_{\text{data}}(x) + p_G(x))/2} \right) \right] + \mathbb{E}_{x \sim p_G} \left[ \log \left( \frac{p_G(x)}{(p_{\text{data}}(x) + p_G(x))/2} \right) \right] - \log(4) \\  &= 2\text{JS}(p_{\text{data}} \| p_G) - \log(4)
  \end{aligned}
  $$
...
- We have the following via Fenchel duality
  \[
  \begin{aligned}
  D_f(p\|q) &= \mathbb{E}_{x \sim q} \left[ f \left( \frac{p(x)}{q(x)} \right) \right] \\  &= \mathbb{E}_{x \sim q} \left[ f^{**} \left( \frac{p(x)}{q(x)} \right) \right] \\  &= \mathbb{E}_{x \sim q} \left[ \sup_t \left( t \times \frac{p(x)}{q(x)} - f^*(t) \right) \right] \\  &= \mathbb{E}_{x \sim q} \left[ \left( T^*(x) \times \frac{p(x)}{q(x)} - f^*(T^*(x)) \right) \right] \\  &= \int q(x) \left( T^*(x) \times \frac{p(x)}{q(x)} - f^*(T^*(x)) \right) dx \\  &= \int \left( T^*(x)p(x) - f^*(T^*(x))q(x) \right) dx \\  &= \sup_T \int \left( T(x)p(x) - f^*(T(x))q(x) \right) dx \\  &= \sup_T \left( \mathbb{E}_{x \sim p}[T(x)] - \mathbb{E}_{x \sim q}[f^*(T(x))] \right)
  \end{aligned}
  \]
  where $T$ denotes a mapping from $\mathcal{X} = \mathbb{R}^d$ to $\mathbb{R}$

### f-GAN与Vanilla GAN的关系
