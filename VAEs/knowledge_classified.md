...
## 知识点2：近似后验推断 (Approximate Posterior Inference)
### 2.1 基本概念
...
### 3.2 证据下界 (Evidence Lower Bound, ELBO)
...
- For training data $\mathbf{x}_1, \dots, \mathbf{x}_N$, the optimization problem is:
  $$\begin{aligned}
  \min_{\theta, \phi} \ &\sum_{i=1}^N \left( \frac{1}{M_i} \sum_{m=1}^{M_i} \frac{\|x - f_\theta(\boldsymbol{\mu}_\phi(\mathbf{x}_i) + \sigma_\phi(\mathbf{x}_i)\boldsymbol{\epsilon}_{i,m})\|_2^2}{2\sigma_{\text{dec}}^2} \right) \\  &+ \sum_{i=1}^N \frac{1}{2} \left( k\sigma_\phi^2(\mathbf{x}_i) + \|\boldsymbol{\mu}_\phi(\mathbf{x}_i)\|_2^2 - k\log\left(\sigma_\phi^2(\mathbf{x}_i)\right) \right)
  \end{aligned}$$
